...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # First layer
    scores_first_layer = np.dot(X, W1) + b1

    # ReLU
    scores_first_layer_relu = np.maximum(0, scores_first_layer)

    # Second layer
    scores = np.dot(scores_first_layer_relu, W2) + b2
    ...
    # Loss
    loss = -np.sum(np.log(softmax_scores[range(N), y])) / N
  ...
  def predict(self, X):
    ...
    # First layer
    scores_first_layer = np.dot(X, self.params["W1"]) + self.params["b1"]

    # ReLU
    scores_first_layer_relu = np.maximum(0, scores_first_layer)

    # Second layer
    scores = np.dot(scores_first_layer_relu, self.params["W2"]) + self.params["b2"]
