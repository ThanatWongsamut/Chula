...
object Q2 {
  def partialMap(l:List[String])(f1:String => String)(f2: String => Boolean): List[String] = {
    @tailrec
    def solver(l:List[String], storage:List[String]): List[String] = {
      if (l.isEmpty) { return storage }

      if (f2(l.head)) {
        solver(l.tail, storage ++ List(f1(l.head)))
      } else {
        solver(l.tail, storage ++ List(l.head))
      }
    }
